...
class discretization(object):
    ...
    def __init__(self, bounds, boxes):
        ...
        self._h = _numpy.divide(bounds[:, 1] - bounds[:, 0], boxes) # length of interval in each direction
    ...
    def randPerBox(self, n):
        ...
        x = _numpy.zeros([d, nTestPoints]) # for the test points
        for i in range(nBoxes):
             index = indexS2M(i, self._boxes) # corresponding multi-index
             lb = self._bounds[:, 0] + _numpy.multiply(index,    self._h) # lower bounds for box
             ub = self._bounds[:, 0] + _numpy.multiply(index +1, self._h) # upper bounds for box
             for mu in range(d):
    ...
    def midpointGrid(self):
        ...
        x = []
        for i in range(d):
            x.append( _numpy.linspace(b[0, 0] + h[0]/2, b[0, 1] - h[0]/2, self._boxes[0]) )
        X = _scipy.meshgrid(*x, indexing = 'ij')
        c = _numpy.zeros([d, self.numBoxes()])
        for i in range(d):
...
def randb(n, b):
    ...
    return b[0] + (b[1] - b[0])*_scipy.rand(1, n)
